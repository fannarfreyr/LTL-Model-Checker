...
def make_schedule(items = ['wolf','cabbage','goat','spinach','popeye','computer','wine'],length = 1000,loop = 420):
	lasso = length - loop
	sched = {}
	side = {}
	emp = 'employee'
...
# Corrections:
	...
	for i in range(length):
		...
		for thing in items:
			if not (side[thing][i] == n):
				current += thing + side[thing][i] + ' '
		current += emp + side[emp][i]
